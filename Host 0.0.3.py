...
if not (os.path.exists(CERT_FILE) and os.path.exists(KEY_FILE)):
    print(f"⚠️ ADVERTENCIA: No se encontraron certificados en {CERT_DIR}")
...
def main():
    ...
    try:
        ...
        print(f"✅ Certificados SSL cargados desde {CERT_DIR}")
    except Exception as e:
        print(f"❌ Error SSL: {e}")
        print("   El servidor no iniciará sin certificados válidos.")
        return
    
    # Configurar socket para escuchar en todas las interfaces
    ...
    s.bind((HOST, PORT))
    s.listen(5)
    print(f"📌 [SERVIDOR COMPLETO] Escuchando en {HOST}:{PORT}")
